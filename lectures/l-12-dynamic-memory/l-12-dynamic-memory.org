...
** We can avoid the exception if we check the return value
...
- A *std::shared_pointer* does everything a bare pointer does
  ...
  - i.e. there is no *std::shared_pointer* that points to the memory any more
...
- *the declaration does not allocate the pointer!*
...
operator = *not defined* for the class std::shared_ptr
...
- allocates and initializes memory as required
