...
| clear     | reset the string to "" (empty string)                 |
...
| push_back | add one character to the end of a string              |
| pop_back  | remove last character (if it exists)                  |
| +=        | append another string or a character to a string      |
| erase     | remove some characters from the string                |
| insert    | insert a substring or a character in a given location |
...
This is the input to read in the following examples.
